...
class PlannerProblemGoal(M.Edge):
    def __init__(self, problem):
        self.result = M.Head(M.Tail(M.Tail(M.Tail(problem)())())())()
...
class Induction(M.Edge):
    """
    Induction method: prove `property_pattern` for every Nat by proving it
    at `base_value` and proving the step under the hypothesis.

    property_pattern is a fact template over the induction variable
    `variable` (a VarTag pattern). The method is planner data only --
    never inserted into mathematical Knowledge. The split it licenses is
    the one genuinely non-saturating move of rung 2: base case and step
    case become ordinary obligations for the existing prove machinery,
    and the step case's hypothesis is recorded knowledge, exactly like a
    trainer-supplied fact.
    """

    def __init__(self, variable, base_value, property_pattern):
        self.result = M.Pair(
            L.InductionLabel,
            M.Pair(variable, M.Pair(base_value, M.Pair(property_pattern, M.EmptyList))),
        )
        super().__init__(
            inputs=M.Pair(variable, M.Pair(base_value, M.Pair(property_pattern, M.EmptyList))),
            results=self.result,
        )
...
class InductionObligations(M.Edge):
    """
    Obligations generated by an Induction(variable, base, pattern) method.

    Two, in order:

      BaseCase(pattern[variable := base])
          the property at the base value, as a closed goal;

      StepCase(pattern[variable := k], pattern[variable := Succ(k)])
          the property at a fresh symbolic k as recorded hypothesis, the
          property at Succ(k) as goal. `step_symbol` is caller-supplied:
          the caller owns freshness, because this substrate has no binder
          discipline yet -- an Induction obligation is honest only when
          its k appears nowhere in the surrounding problem. Recorded as a
          term so the gate can check it, not silently assumed.

    Discharge is directed, never saturating: the rung-1 gate measured
    generative-lemma saturation diverging, so the step case must consume
    its hypothesis through explicit application.
    """

    def __init__(self, method, step_symbol):
        empty = M.EmptyList
        fields = M.Tail(method)()
        variable = M.Head(fields)()
        fields = M.Tail(fields)()
        base_value = M.Head(fields)()
        pattern = M.Head(M.Tail(fields)())()

        base_binding = M.Pair(
            M.Pair(variable, M.Pair(base_value, empty)),
            empty,
        )
        base_fact = M.Head(M.Instantiate(pattern, base_binding)())()

        step_binding = M.Pair(
            M.Pair(variable, M.Pair(step_symbol, empty)),
            empty,
        )
        hypothesis_fact = M.Head(M.Instantiate(pattern, step_binding)())()
        successor = M.Pair(L.SuccLabel, M.Pair(step_symbol, empty))
        successor_binding = M.Pair(
            M.Pair(variable, M.Pair(successor, empty)),
            empty,
        )
        successor_fact = M.Head(M.Instantiate(pattern, successor_binding)())()

        self.result = M.Pair(
            M.Pair(L.BaseCaseLabel, M.Pair(base_fact, empty)),
            M.Pair(
                M.Pair(
                    L.StepCaseLabel,
                    M.Pair(hypothesis_fact, M.Pair(successor_fact, empty)),
                ),
                empty,
            ),
        )
        super().__init__(
            inputs=M.Pair(method, M.Pair(step_symbol, empty)),
            results=self.result,
        )
...
class PigeonholeObligations(M.Edge):
    """
    Obligations generated by a Pigeonhole(P, B, f) method.

    Finiteness of both sides, totality of the assignment, both cardinalities,
    and the strict inequality that makes the box principle bite. The overloaded
    box is never searched for.
    """

    def __init__(self, method, domain_size, codomain_size):
        empty = M.EmptyList
        fields = M.Tail(method)()
        domain = M.Head(fields)()
        fields = M.Tail(fields)()
        codomain = M.Head(fields)()
        assignment = M.Head(M.Tail(fields)())()
        self.result = M.Pair(
            M.Pair(L.FiniteLabel, M.Pair(domain, empty)),
            M.Pair(
                M.Pair(L.FiniteLabel, M.Pair(codomain, empty)),
                M.Pair(
                    M.Pair(L.TotalMapLabel, M.Pair(assignment, M.Pair(domain, M.Pair(codomain, empty)))),
                    M.Pair(
                        M.Pair(L.CardinalityLabel, M.Pair(domain, M.Pair(domain_size, empty))),
                        M.Pair(
                            M.Pair(L.CardinalityLabel, M.Pair(codomain, M.Pair(codomain_size, empty))),
                            M.Pair(
                                M.Pair(L.NatLessLabel, M.Pair(codomain_size, M.Pair(domain_size, empty))),
                                empty,
                            ),
                        ),
                    ),
        ...
        super().__init__(
            inputs=M.Pair(method, M.Pair(domain_size, M.Pair(codomain_size, empty))),
            results=self.result,
        )
...
class PigeonholeConclusion(M.Edge):
    """
    Assembly conclusion of the box principle: two distinct elements of the
    domain that the assignment sends to one box. The witnesses stay symbolic.
    """

    def __init__(self, method, left_witness, right_witness, box):
        empty = M.EmptyList
        fields = M.Tail(method)()
        domain = M.Head(fields)()
        fields = M.Tail(fields)()
        ...
        collision = M.Pair(
            L.CollisionLabel,
            M.Pair(
                M.Pair(L.InLabel, M.Pair(left_witness, M.Pair(domain, empty))),
                M.Pair(
                    M.Pair(L.InLabel, M.Pair(right_witness, M.Pair(domain, empty))),
                    M.Pair(
                        M.Pair(L.ApartLabel, M.Pair(left_witness, M.Pair(right_witness, empty))),
                        M.Pair(
                            M.Pair(L.ApplyLabel, M.Pair(assignment, M.Pair(left_witness, M.Pair(box, empty)))),
                            M.Pair(
                                M.Pair(L.ApplyLabel, M.Pair(assignment, M.Pair(right_witness, M.Pair(box, empty)))),
                                empty,
                            ),
                        ),
                    ),
        ...
        self.result = M.Pair(L.ExistsLabel, M.Pair(collision, empty))
        super().__init__(
            inputs=M.Pair(method, M.Pair(left_witness, M.Pair(right_witness, M.Pair(box, empty)))),
            results=self.result,
        )
...
class ExtremalObligations(M.Edge):
    """
    Obligations generated by an Extremal(A, measure, direction, variation)
    method against a goal property.

    Attainment is a separate obligation from non-emptiness: a nonempty family
    does not by itself carry an extremal element, only finiteness or a
    well-ordering does.
    """

    def __init__(self, method, witness, property_of_witness):
        empty = M.EmptyList
        fields = M.Tail(method)()
        family = M.Head(fields)()
        fields = M.Tail(fields)()
        measure = M.Head(fields)()
        fields = M.Tail(fields)()
        direction = M.Head(fields)()
        variation = M.Head(M.Tail(fields)())()
        improved = M.Pair(L.VariationLabel, M.Pair(variation, M.Pair(witness, empty)))
        better = M.Pair(
            L.BetterLabel,
            M.Pair(measure, M.Pair(direction, M.Pair(improved, M.Pair(witness, empty)))),
        )
        improving_step = M.Pair(
            L.ExistsLabel,
            M.Pair(
                M.Pair(
                    L.NotLabel,
                    M.Pair(M.Pair(property_of_witness, M.Pair(witness, empty)), empty),
                ),
                M.Pair(better, empty),
            ),
        )
        self.result = M.Pair(
            M.Pair(L.NonEmptyLabel, M.Pair(family, empty)),
            M.Pair(
                M.Pair(L.FiniteLabel, M.Pair(family, empty)),
                M.Pair(
                    M.Pair(L.AttainsLabel, M.Pair(family, M.Pair(measure, M.Pair(direction, empty)))),
                    M.Pair(
                        M.Pair(L.InLabel, M.Pair(witness, M.Pair(family, empty))),
                        M.Pair(
                            M.Pair(
                                L.ExtremalAtLabel,
                                M.Pair(family, M.Pair(measure, M.Pair(direction, M.Pair(witness, empty)))),
                            ),
                            M.Pair(improving_step, empty),
                        ),
                    ),
                ),
            ),
        ...
        super().__init__(
            inputs=M.Pair(method, M.Pair(witness, M.Pair(property_of_witness, empty))),
            results=self.result,
        )
...
class ExtremalConclusion(M.Edge):
    """
    Assembly of the extremal argument. An improving variation contradicts
    extremality, so the extremal element carries the property and the
    existential follows.
    """
    ...
    def __init__(self, method, witness, property_of_witness):
        ...
        fields = M.Tail(fields)()
        measure = M.Head(fields)()
        direction = M.Head(M.Tail(fields)())()
        extremal_at = M.Pair(
            L.ExtremalAtLabel,
            M.Pair(family, M.Pair(measure, M.Pair(direction, M.Pair(witness, empty)))),
        )
        holds = M.Pair(property_of_witness, M.Pair(witness, empty))
        self.result = M.Pair(
            L.ExistsLabel,
            M.Pair(
                holds,
                M.Pair(
                    extremal_at,
                    M.Pair(M.Pair(L.ContradictionLabel, M.Pair(extremal_at, empty)), empty),
                ),
            ),
        )
        super().__init__(
            ...
            while M.IdentityCompare(remaining_obligations, M.EmptyList)() is M.false_value:
                ...
                if M.AndAtom(
                    ...
                    if M.IdentityCompare(problem, M.EmptyList)() is M.false_value:
                        ...
                        # Trainer-supplied Engel methods become alternatives
                        # beside the ordinary rule decompositions. Only
                        # Pigeonhole and Extremal are expanded so far; other
                        # method terms are carried but generate no children.
                        method_decomposed = M.false_value
                        already_has_alternatives = M.false_value
                        existing_alternatives = alternatives
                        while M.IdentityCompare(existing_alternatives, M.EmptyList)() is M.false_value:
                            if M.TermEqual(
                                PlannerAlternativeParent(M.Head(existing_alternatives)())(),
                                obligation_id,
                            )() is M.truth_value:
                                already_has_alternatives = M.truth_value
                            existing_alternatives = M.Tail(existing_alternatives)()
                        # Trainer methods address the problem's own goal, not
                        # the child obligations a method has already spawned.
                        remaining_methods = M.EmptyList
                        if M.TermEqual(obligation_goal, PlannerProblemGoal(problem)())() is M.truth_value:
                            remaining_methods = PlannerProblemMethods(problem)()
                        if M.IdentityCompare(already_has_alternatives, M.truth_value)() is M.truth_value:
                            remaining_methods = M.EmptyList
                        while M.IdentityCompare(remaining_methods, M.EmptyList)() is M.false_value:
                            ...
                            if M.IdentityCompare(method_head, L.PigeonholeLabel)() is M.truth_value:
                                domain_size = M.Pair(
                                    L.CardinalityLabel,
                                    M.Pair(M.Head(M.Tail(method)())(), M.EmptyList),
                                )
                                codomain_size = M.Pair(
                                    L.CardinalityLabel,
                                    M.Pair(M.Head(M.Tail(M.Tail(method)())())(), M.EmptyList),
                                )
                                method_goals = PigeonholeObligations(method, domain_size, codomain_size)()
                            if M.IdentityCompare(method_head, L.ExtremalLabel)() is M.truth_value:
                                extremal_witness = M.Pair(
                                    L.ExtremalAtLabel,
                                    M.Pair(M.Head(M.Tail(method)())(), M.EmptyList),
                                )
                                method_goals = ExtremalObligations(method, extremal_witness, obligation_goal)()
                            if M.IdentityCompare(method_goals, M.EmptyList)() is M.false_value:
                                method_children = M.EmptyList
                                goals_to_add = method_goals
                                while M.IdentityCompare(goals_to_add, M.EmptyList)() is M.false_value:
                                    ...
                                    method_children = M.Pair(child_id, method_children)
                                    goals_to_add = M.Tail(goals_to_add)()
                                reversed_method_children = M.EmptyList
                                while M.IdentityCompare(method_children, M.EmptyList)() is M.false_value:
                                    reversed_method_children = M.Pair(M.Head(method_children)(), reversed_method_children)
                                    method_children = M.Tail(method_children)()
                                alternatives = M.Pair(
                                    PlannerAlternative(
                                        obligation_id,
                                        method,
                                        reversed_method_children,
                                        L.PendingLabel,
                                        M.EmptyList,
                                    )(),
                                    alternatives,
                                )
                                method_decomposed = M.truth_value
                            remaining_methods = M.Tail(remaining_methods)()
                        if M.IdentityCompare(method_decomposed, M.truth_value)() is M.truth_value:
