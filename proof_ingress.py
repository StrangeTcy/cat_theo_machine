...
rewrite-rule VarTags. Comparisons admit =, <, >, >=, and <=; the inclusive
operators stay distinct heads (ge/le) so a >= bound is never silently
turned into a strict one. Two no-solutions phrasings are recognized:
"has no solutions in <domain>" and "is not solvable for <names> which are
<domain>", the latter binding exactly the listed names. The grammar admits
one outer universal scope and an inner no-solutions scope;
...
class ProofTokenStream(M.Edge):
    ...
    def __init__(self, text):
        ...
        while reading is M.truth_value:
            ...
            consumed_position = next_position
                ...
                if reading is M.truth_value and not character.isspace():
                    ...
                    if M.Compare(kind, M.Char("symbol"))() is M.truth_value:
                        ...
                        if character == ">" or character == "<":
                            ...
                            if following == "=":
                                run = character + following
                                end_position = G.GMPSuccText(next_position)()
                                consumed_position = end_position
                            elif following != "":
                                stream.seek(stream.tell() - 1)
                        reversed_tokens = M.Pair(M.Pair(M.Char(run), M.Pair(kind, M.Pair(M.GMPRep(start), M.Pair(M.GMPRep(end_position), M.EmptyList)))), reversed_tokens)
                        run = ""
...
class ProofComparison(M.Edge):
    def __init__(self, state, left):
        self.result = M.EmptyList
        operator = state.word
        allowed = M.Pair(M.Char("="), M.Pair(M.Char("<"), M.Pair(M.Char(">"), M.Pair(M.Char(">="), M.Pair(M.Char("<="), M.EmptyList)))))
...
class MathematicalSentence(M.Edge):
    ...
    def __init__(self, tokens):
        ...
        guard = M.EmptyList
        if M.Compare(state.word, M.Char(">"))() is M.truth_value or M.Compare(state.word, M.Char("<"))() is M.truth_value or M.Compare(state.word, M.Char(">="))() is M.truth_value or M.Compare(state.word, M.Char("<="))() is M.truth_value:
        ...
        if M.Compare(state.word, M.Char("has"))() is M.truth_value:
            ...
            no_solutions = M.truth_value
        elif M.Compare(state.word, M.Char("is"))() is M.truth_value:
            ProofAdvance(state)()
            ProofExpect(state, M.Char("not"))()
            ProofExpect(state, M.Char("solvable"))()
            ProofExpect(state, M.Char("for"))()
            listed = M.EmptyList
            if state.failure is M.EmptyList:
                first_name = ProofName(state)()
                if G.SurfaceChainHasWord(listed, first_name)() is M.truth_value:
                    ProofFailure(state, M.Char("each solvable-for variable once"))()
                else:
                    listed = M.Pair(first_name, listed)
            while state.failure is M.EmptyList and M.Compare(state.word, M.Char(","))() is M.truth_value:
                ProofAdvance(state)()
                next_name = ProofName(state)()
                if G.SurfaceChainHasWord(listed, next_name)() is M.truth_value:
                    ProofFailure(state, M.Char("each solvable-for variable once"))()
                else:
                    listed = M.Pair(next_name, listed)
            ProofExpect(state, M.Char("which"))()
            ProofExpect(state, M.Char("are"))()
            solution_domain = ProofDomain(state)()
            names = M.Reverse(listed)()
            if state.failure is M.EmptyList:
                remaining = names
                while remaining is not M.EmptyList:
                    if G.SurfaceChainHasWord(state.free, M.Head(remaining)())() is M.false_value:
                        ProofFailure(state, M.Char("solvable-for variable does not occur in the equation"))()
                    remaining = M.Tail(remaining)()
                remaining = state.free
                while remaining is not M.EmptyList:
                    if G.SurfaceChainHasWord(names, M.Head(remaining)())() is M.false_value:
                        ProofFailure(state, M.Char("equation variable missing from the solvable-for list"))()
                    remaining = M.Tail(remaining)()
            unknowns = M.Pair(M.Char("unknowns"), names)
            no_solutions = M.truth_value
        ...
        if no_solutions is M.truth_value:
...
class ProofCommandEnvelope(M.Edge):
    """Recognize the command envelope without running the claim grammar.

    The live entrypoint announces the envelope before parsing, so a
    recognized proof line is reported even when its claim later fails or
    requests clarification. Only the words prove and that are read here.
    """
