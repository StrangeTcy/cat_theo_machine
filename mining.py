...
class GMPPowerText(M.Edge):
    def __init__(self, base_text, exponent_text):
        if GMPEqualText(exponent_text, "0")() is M.truth_value:
            self.result = "1"
        else:
            self.result = GMPMulText(
                base_text,
                GMPPowerText(base_text, GMPPredText(exponent_text)())(),
            )()
        super().__init__(
            inputs=M.Pair(M.Char(base_text), M.Pair(M.Char(exponent_text), M.EmptyList)),
...
class EvaluateBivariatePolynomialText(M.Edge):
    def __init__(self, polynomial, first_text, second_text):
        if M.IdentityCompare(polynomial, M.EmptyList)() is M.truth_value:
            self.result = "0"
        else:
            monomial = M.Head(polynomial)()
            monomial_value = GMPMulText(
            ...
            self.result = GMPAddText(
                monomial_value,
                EvaluateBivariatePolynomialText(
                    M.Tail(polynomial)(), first_text, second_text,
                )(),
            )()
        super().__init__(inputs=M.Pair(polynomial, M.EmptyList), results=M.Char(self.result))

    def __call__(self):
        return self.result


class BoundedPolynomialCounterexampleGrid(M.Edge):
    def __init__(self, lesser, greater, first_text="-2", second_text="-2"):
        if GMPEqualText(first_text, "3")() is M.truth_value:
            self.result = M.EmptyList
        elif GMPEqualText(second_text, "3")() is M.truth_value:
            self.result = BoundedPolynomialCounterexampleGrid(
                lesser, greater, GMPSuccText(first_text)(), "-2",
            )()
        else:
            lesser_value = EvaluateBivariatePolynomialText(
                lesser, first_text, second_text,
            )()
            greater_value = EvaluateBivariatePolynomialText(
                greater, first_text, second_text,
            )()
            if GMPLessText(greater_value, lesser_value)() is M.truth_value:
                self.result = M.Pair(
                    M.Char("polynomial-counterexample"),
                    M.Pair(
                        M.Char(first_text),
                        M.Pair(
                            M.Char(second_text),
                            M.Pair(
                                M.Char(lesser_value),
                                M.Pair(M.Char(greater_value), M.EmptyList),
                            ),
                        ),
                    ),
                )
                ...
                self.result = BoundedPolynomialCounterexampleGrid(
                    lesser, greater, first_text, GMPSuccText(second_text)(),
                )()
...
class BoundedPolynomialCounterexample(M.Edge):
    def __init__(self, goal, registry=M.EmptyList):
        self.result = M.EmptyList
        ...
        if M.IdentityCompare(variables, M.EmptyList)() is M.false_value:
            if M.IdentityCompare(M.Tail(variables)(), M.EmptyList)() is M.false_value:
                ...
                lesser = NormalizePolynomial(
                    M.Head(M.Tail(goal)())(), first, second, registry,
                )()
                greater = NormalizePolynomial(
                    M.Head(M.Tail(M.Tail(goal)())())(), first, second, registry,
                )()
                self.result = BoundedPolynomialCounterexampleGrid(
                    lesser, greater,
                )()
