...
class DefinitionProductionTest(M.Edge):
    ...
    def __init__(self, graph):
        ...
        one = M.Char("one")
        prime_concept = M.Char("prime")
        nat_category = M.Char("nat")
        divisor_role = M.Char("divisor")
        number_chunk = M.Pair(
            Gmod.CategoryTerm(nat_category)(),
            M.Pair(M.Pair(Lmod.NonNegativeLabel, empty), empty),
        )
        entries = {
            "definition": ("CW", M.Char("definition")),
            ":": ("COL", M.Char(":")),
            " ": ("SPC", M.Char(" ")),
            "a": ("DET", M.Char("a")),
            "prime": ("ADJ", prime_concept),
            "number": ("CN", number_chunk),
            "is": ("COP", M.Char("is")),
            "divisible": ("RADJ", Lmod.DividesLabel),
            "only": ("ROP", M.Char("only")),
            "by": ("P", divisor_role),
            "one": ("NUM", one),
            "and": ("CONJ", M.Char("and")),
            "itself": ("RPRON", M.Pair(Lmod.ReflexiveLabel, empty)),
        }
        categories = {}
        for name in (
            "CW", "COL", "SPC", "DET", "ADJ", "CN", "COP", "RADJ", "ROP",
            "P", "NUM", "CONJ", "RPRON", "ADJG", "DETG", "CWG", "CWGG",
            "NP", "NPG", "SBJ", "COPG", "NUMG", "CONJG", "CJPRON", "COORD",
            "PG", "PP", "ROPG", "RPRED", "RADJG", "PRED", "DEF",
        ):
            categories[name] = M.Char(name)
        ...
        for word in entries:
            ...
            for ch in word:
                key = (state.id, ch)
                if key not in states:
                    nxt = M.Char("st:" + word + ":" + str(len(states)))
                    states[key] = nxt
                    arcs_reversed = M.Pair(
                        Gmod.FormArc(state, symbols[ch], nxt)(),
                        arcs_reversed,
        ...
        senses_reversed = empty
        for word, (category_name, meaning) in entries.items():
            senses_reversed = M.Pair(
                Gmod.FormSense(
                    states[("final", word)], categories[category_name], meaning,
                )(),
                senses_reversed,
            )
        senses = M.Reverse(senses_reversed)()
        ...
        production_number = 0
        for left, right, result, template in (
            ("ADJ", "SPC", "ADJG", "left"),
            ("ADJG", "CN", "NP", "lexical-np"),
            ("DET", "SPC", "DETG", "left"),
            ("DETG", "NP", "NP", "project-right"),
            ("CW", "COL", "CWG", "left"),
            ("CWG", "SPC", "CWGG", "left"),
            ("CWGG", "NP", "NP", "project-right"),
            ("NP", "SPC", "NPG", "left"),
            ("COP", "SPC", "COPG", "left"),
            ("NPG", "COPG", "SBJ", "left"),
            ("NUM", "SPC", "NUMG", "left"),
            ("CONJ", "SPC", "CONJG", "left"),
            ("CONJG", "RPRON", "CJPRON", "project-right"),
            ("NUMG", "CJPRON", "COORD", "pair"),
            ("P", "SPC", "PG", "left"),
            ("PG", "COORD", "PP", "restriction"),
            ("ROP", "SPC", "ROPG", "left"),
            ("ROPG", "PP", "RPRED", "project-right"),
            ("RADJ", "SPC", "RADJG", "left"),
            ("RADJG", "RPRED", "PRED", "pair"),
            ("SBJ", "PRED", "DEF", "definition"),
        ):
            first = M.Pair(
                M.VarTag,
                M.Pair(M.Char("?p" + str(production_number) + "a"), empty),
            )
            second = M.Pair(
                M.VarTag,
                M.Pair(M.Char("?p" + str(production_number) + "b"), empty),
            )
            if template == "left":
                built = first
            elif template == "project-right":
                built = M.Pair(
                    Lmod.ProjectRightLabel, M.Pair(second, empty),
                )
            elif template == "pair":
                built = M.Pair(first, M.Pair(second, empty))
            elif template == "lexical-np":
                built = M.Pair(
                    Lmod.LexicalNpLabel,
                    M.Pair(first, M.Pair(second, empty)),
                )
            elif template == "restriction":
                built = M.Pair(
            ...
            productions_reversed = M.Pair(
                Gmod.BinaryProduction(
                    categories[left], categories[right], categories[result],
                    built,
                )(),
        ...
        while M.IdentityCompare(remaining, empty)() is M.false_value:
            reading = M.Head(remaining)()
            category = M.Head(M.Tail(reading)())()
            if M.IdentityCompare(category, categories["DEF"])() is M.truth_value:
                definitions.append((
